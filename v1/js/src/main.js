...
function toggleDarkMode() {
	...
	if (!dark_mode) {
		...
		tl
			...
			.to("html", {
				duration: .128, onComplete: function () {
	...
	init: function () {
		...
		this.el.onclick = function (e) {
			...
			tl
				...
				.fromTo('.menu__pop .menu__item ul li, .menu__pop .menu__item .border', { transformOrigin: "0 0", xPercent: 200, opacity: 0 }, {
					xPercent: 0, opacity: 1, duration: .768, ease: "expo.out", stagger: {
						from: 0,
						amount: .128
					}
				})
		...
		this.el.onclick = function (e) {
			...
			tl
				.to('.menu__pop .menu__item ul li, .menu__pop .menu__item .border', {
...
var loading = {
	...
	show: function (callback) {
		var that = this,
			...
			var_to = {
				xPercent: 0, yPercent: 0, duration: .768, ease: "expo.inOut", onComplete: function () {
					if (dark_mode) {
		...
		if (_q("[data-namespace]").getAttribute("data-namespace") == "call") {
			...
			tl
				...
				.to(sl, {
					position: "fixed", top: 0, right: 0, width: window.innerWidth, height: window.innerHeight, borderRadius: 0, duration: .768, ease: "expo.inOut", onComplete: function () {
	...
	hide: function (callback, callback__half) {
		var that = this,
			...
			var_to = {
				xPercent: 0, yPercent: -100, duration: 1.024, ease: "expo.inOut", onComplete: function () {
					// Move loading
		...
		if (_q("[data-namespace]").getAttribute("data-namespace") == "call") {
			...
			tl
				...
				.to(sl.querySelector(".text"), {
					margin: 0, onComplete: function () {
						addClass(sl, "bubble");
			...
			gsap.fromTo(that.selector, { scale: 1 }, {
				scale: 1, duration: var_to.duration / 3, onComplete: function () {
			...
			gsap.to(window, {
				...
				onComplete: function () {
					...
					if (that.loaded) {
						...
						// Scroll up instantly
						gsap.to(window, {
							duration: 0, scrollTo: 0, onComplete: function () {
								// Hide loading
								loading.hide(
									function () {
	...
	.extend({
		start: function () {
			...
			if (menu.active) {
				...
				animate();
				tl.fromTo('.menu__pop .menu__item ul li, .menu__pop .menu__item .border', { xPercent: 0, opacity: 1 }, {
					xPercent: -200, opacity: 0, stagger: {
						from: 0,
						amount: .128
					}
				});
...
var Home = Barba
	...
	.extend({
		...
		onEnterCompleted: function () {
			...
			_q(".img").onmousedown = function (e) {
				...
				if (!anim.isActive()) {
					...
					anim
						.fromTo(".poof", { transformOrigin: "40%", opacity: 1, rotation: 15, scale: 1, xPercent: -50, yPercent: 50 }, {
							rotation: 0, scale: 1.75, xPercent: 0, yPercent: 0, onComplete: function () {
		...
		onImageLoadComplete: function () {
			...
			anim
				.fromTo(".work__list > .block", { y: 200 }, {
					y: 0, stagger: {
						from: 0,
						amount: .128
					}
				}, 0)
				.fromTo(".work__list .stats__content p", { y: 200 }, {
					y: 0, stagger: {
						from: "end",
						amount: .128
					}
			...
			new ScrollMagic
				...
				.addTo(controller);
			// Work list
			anim = gsap.fromTo(".work__list .scroller, .gallery a:not(:last-child)", { y: 200 }, {
				y: 0, duration: .768, ease: "expo.out", stagger: {
					from: 0,
					amount: .128
				}
			});
		...
		onImageLoadAnimateHalfComplete: function () {
			...
			anim
				...
				.fromTo(".hero__text h1 .splext", { y: 300 }, {
					y: 0, stagger: {
						from: 0,
						amount: .064
					}
				...
				.fromTo(".hero__text p .splext", { y: 300 }, {
					y: 0, stagger: {
						from: 0,
						amount: .128
					}
				}, .128)
				.fromTo(".hero__text p, .hero .stats", { transformOrigin: "0 0", y: 500 }, {
					y: 0, stagger: {
						from: 0,
						amount: .256
					}
				}, .256)
				.fromTo(".hero .stats p", { y: 250 }, {
					y: 0, stagger: {
						from: "end",
						amount: .256
					}
...
document.addEventListener('DOMContentLoaded', function (e) {
	...
	imageloading.init(document, function (imgload) {
		...
		loading.animate((_q(".barba-container").getAttribute("data-namespace") == "404" ? "404" : "first-time"), function () {
			...
			// Animate header
			gsap.fromTo(".logo, .menu > a, #mode, .lang > li", { marginTop: -250 }, {
				marginTop: 0, delay: .512, duration: 2.048, ease: "expo.out", stagger: {
					from: 0,
					amount: 1.024
				}
			});
